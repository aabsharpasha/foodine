 ...
 * @author KelltonTech Solutions Ltd
 */
    ...
    function getRestaurantDetail_post() {
        try {
            ...
            if (checkselectedparams($this->post(), $allowParam)) {
                ...
                $STATUS = FAIL_STATUS;
                //GET DETAIL
                $rec = $this->restaurant_model->getRestaurantDetail($this->post('restaurantId'), $this->post('userLat'), $this->post('userLong'), $this->post('userId'), $this->post('platform'));
                if (isset($rec['info']['restaurantId']) && !empty($rec['info']['restaurantId'])) {
                    $MESSAGE = RESTAURANT_FOUND;
                    $STATUS = SUCCESS_STATUS;
                    $RESTAURANTDETAIL = $rec;
                }
            }
            $resp = array(
                'MESSAGE' => $MESSAGE,
            ...
            if (@$RESTAURANTDETAIL !== '') {
                $resp['RESTAURANTDETAIL'] = $RESTAURANTDETAIL;
            }
            //RESPONSE
            $this->response($resp, 200);
        } catch (Exception $ex) {
...
//                    $this->response($data, 200);
    ...
    public function getBanner_post() {
        ...
        try {
            ...
            $resp = $this->restaurant_model->getBanner();
            if (!empty($resp)) {
                $STATUS = SUCCESS_STATUS;
