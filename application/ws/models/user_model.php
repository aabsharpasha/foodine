    ...
    public function getCIMUserArray($type = 'cuisine', $userId = '') {
        ...
        switch ($type) {
            case 'cuisine' :
                ...
                if ($userId !== '') {
                    $fields[] = 'IF((SELECT COUNT(*) FROM tbl_user_cuisine AS tuc WHERE tuc.iCuisineID = tc.iCuisineID AND tuc.iUserID = "' . $userId . '") = "0", "no", "yes") AS selected';
                } else {
            ...
            case 'interest' :
                ...
                if ($userId !== '') {
                    $fields[] = 'IF((SELECT COUNT(*) FROM tbl_user_interest AS tui WHERE tui.iInterestID = tf.iFacilityID AND tui.iUserID = "' . $userId . '") = "0", "no", "yes") AS selected';
                } else {
                    $fields[] = '"no" AS selected';
                }
                break;
            ...
            case 'music' :
                ...
                if ($userId !== '') {
                    $fields[] = 'IF((SELECT COUNT(*) FROM tbl_user_music AS tum WHERE tum.iMusicID = tm.iMusicID AND tum.iUserID = "' . $userId . '") = "0", "no", "yes") AS selected';
                } else {
                    $fields[] = '"no" AS selected';
                }
                break;
...
//                $this->_addUserCIMValues($userId, 'category', $userCategory);
    ...
    function userBookmarkList($userId = '') {
        try {
            if ($userId != '') {
                $sql = 'SELECT `turf`.`iFavoriteID` AS bookmarkId, `turf`.iRestaurantID as id, `tr`.`vRestaurantName` AS restaurantName, ';
                ...
                $sql .= ' CONCAT("' . BASEURL . 'images/restaurant/", IF(`tr`.vRestaurantLogo = \'\', "default.png", CONCAT(`tr`.iRestaurantID,\'/\',`tr`.vRestaurantLogo)) ) AS photoUrl, ';
        ...
        try {
            if (!empty($postValues)) {
               ...
               // $sql .= 'trr.iAmbience as ambience, trr.iPrice as price, trr.iFood as food, trr.iService as service,';
                ...
                $sql .= ' trr.tReviewDetail AS reviewComment, UNIX_TIMESTAMP(trr.tCreatedAt) AS dateTime,';
...
//print_r($sql);exit;
    ...
    function updateUserChoice($userId, $cuisine, $interest, $priceInfo) {
        try {
