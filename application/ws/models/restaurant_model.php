...
//            $sql .= ' AVG(`trr`.iRateValue) AS rating, `tr`.eAlcohol AS alcohol, `tr`.vDaysOpen as openDays, `tr`.tSpecialty as speciality ';
     ...
     * 
     * @modified by Garima <garima.chowrasia@kelltontech.com>
    ...
    function getRestaurantDetail($recordId, $userLat = '', $userLong = '', $userId = '', $platform = '') {
        ...
        try {
            if ($recordId != '') {
                ...
                $fields[] = 'IFNULL((SELECT AVG(tur.iRateValue) FROM tbl_user_ratting AS tur WHERE tur.iRestaurantID  = tr.iRestaurantID),0) AS restaurantRatting';
        ...
        try {
            if (!empty($postValue)) {
                ...
                if ($userId !== '') {
                    ...
                    $field[] = 'tr.vRewardTitle AS rewardTitle';
                    $field[] = 'tr.tRewardDesc AS rewardDesc';
                    $field[] = 'DATE_FORMAT(tr.dtRewardValidUpto,"' . MYSQL_DATE_FORMAT . '") AS rewardValidDate';
                    $field[] = 'tr.tRewardDesc AS rewardDesc';
                    $field[] = 'tr.iRewardPoint AS rewardPoint';
                    $field[] = 'tr.iRewardVoucher AS rewardVoucher';
                    $field[] = 'CONCAT("' . BASEURL . 'images/reward/", IF(tr.vRewardImage = "", "default.jpeg", CONCAT(tr.iRewardID,"/",tr.vRewardImage))) AS rewardImage';
...
//echo $qry; exit;
    ...
    function getFeaturedRestaurant($userId, $userLat, $userLong, $limit = '') {
       // echo $userLat; exit;
        $query = 'SELECT tr.`iRestaurantID` AS Id, tr.vRestaurantName, IF(tr.eFeatured=\'yes\',\'yes\',"") AS eFeatured FROM tbl_restaurant AS tr WHERE tr.eStatus = \'Active\' ORDER BY eFeatured DESC, tr.vRestaurantName ASC limit '.$limit;
        ...
        if ($res->num_rows() > 0) {
            foreach ($res->result_array() as $rowData) {
              ...
              $resRet['rating'] = $resDetail['info']['userRating'];
              $resRet['bookmarked'] = $resDetail['isFavourite'] == 'no' ? "false" : "true" ;
    ...
    function getRestaurantComboOffers($postValue) {
        try {
            ...
            $fields .= 'tco.iRestaurantID as Id, tr.vRestaurantName as  restaurantName, tco.vOfferText as description, CONCAT(tr.vCityName,\', \',tc.vLocationName) as address,  tcso.tActualPrice as price, tcso.tDiscountedPrice discountedPrice';
...
//                $postValue["rawData"]["testComboData"] = $postValue["comboData"]; // for testing combo data
                ...
                $param = array(
                    '%MAILSUBJECT%' => 'HungerMafia : Order Details',
                ...
                $subject = 'HungerMafia : Order Details';
                $to = $res['vEmail'];
           ...
           // $fields[] = 'tb.tText AS bannerText';
            ...
            switch ($type) {
                case 'featured':
                    ...
                    $fields[] = 'CONCAT("' . BASEURL . 'images/restaurant/", IF(tr.vRestaurantLogo = \'\', "default.jpeg", CONCAT(tr.iRestaurantID,\'/thumb/\',tr.vRestaurantLogo)) ) AS bannerImage';
                    $fields[] = 'CONCAT("' . BASEURL . 'images/restaurant/", IF(tr.vRestaurantLogo = \'\', "default.jpeg", CONCAT(tr.iRestaurantID,\'/thumb/\',tr.vRestaurantLogo)) ) AS restaurantImage';
                    $condition[] = "tr.eStatus = 'Active'";
                ...
                case 'deals':
                    ...
                    $condition[] = "td.eStatus = 'Active'";
                    $condition[] = 'CURDATE() between td.dtStartDate and td.dtExpiryDate';
                    $join .= " LEFT JOIN `tbl_deals` AS `td` ON `tb`.`iTypeId` = `td`.`iDealID`";
               ...
               // $this->load->helper('solr_helper');
                ...
                switch ($searchType) {
                    ...
                    case 'featured':
                        $userLat = $latitude;
                        $userLong = $longitude;
                        $resp = $this->getFeaturedRestaurant($userId, $userLat, $userLong, 20);
                        $res['totalCount'] = 20;
                        break;
                    case 'cuisine' :
                        $data = $this->_solrGetList($postValue);
    ...
    private function _solrGetList($postValue = array()) {
        ...
        try {
            if (!empty($postValue)) {
                ...
                if ($searchType != 'category' && $searchType != 'cuisine' &&  $searchType != 'cuisinerest') {
                    $tbl = 'tbl_restaurant AS tr';
                    $fields .= 'tr.iRestaurantID AS restaurantId';
                }
