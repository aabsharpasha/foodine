...
class Login extends REST_Controller {
    ...
    function index_post() {
      ...
      //  print_r($this->post()); exit;
        if (checkselectedparams($this->post(), array('email', 'password', 'plateForm'))) {
//echo 'hsdafdasf'; exit;
            /*
...
//print_r($result); exit;
    ...
    private function _returnStatus($USERDATA, $status = 'Active', $isVendor = FALSE) {
        try {
            switch ($status) {
                case 'Inactive' :
//                    $resp = array(
...
//                    );
                    $resp = array(
                        ...
                        ($isVendor ? 'VENDORDATA' : 'USERDATA') => $USERDATA
                    );
                    break;
                ...
                case 'Active' :
                   ...
                   if($USERDATA['userDOB']) {
                    $resp['dateOfBirth'] = $USERDATA['userDOB']; 
                   } 
                   
                   if($USERDATA['userAnniversary']) {
                    $resp['anniversary'] = $USERDATA['userAnniversary'];
                   }
		   
                    $resp = array_merge($resp_main, $resp);
