...
//        $this->response($this->post(), 200);
        try {
            ...
            if (checkselectedparams($this->post(), array('pageId'))) {
                ...
                if (!empty($result)) {
                    $resp = array(
                        'result' => 'Record founded successfully.',
                        'resultCode' => SUCCESS_STATUS,
                        'aboutUs' => $result['pageContent'],
...
//        $this->response($this->post(), 200);
        try {
             ...
             * TO CHECK ALL THE PARAMETER ARE BEING PASSED OR NOT ?
             */
            if (checkselectedparams($this->post(), array('subject', 'message', 'email', 'name'))) {
                /*
                 * TO CALL THE FUNCTION FROM THE PAGE-CONTENT-MODEL
                 */
                $result = $this->pagecontent_model->feedback($this->post('subject'), $this->post('message'), $this->post());
                if (!empty($result)) {
                    $resp = array(
