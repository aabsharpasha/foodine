...
class MailLib extends PHPMailer {
    ...
    private function _constantConfig() {
        try {
            define('MAIL_FROM_EMAIL', 'info@blacklist.com');
    ...
    function sendMail($RECIEVER = array(), $SUBJECT = MAIL_SUBJECT, $TEMPLATE = '', $PARAM = array(), $isHTML = FALSE, $ATTACHMENT = array(), $CC = array(), $FROM = array()) {
        try {

            if (!empty($FROM)) {
                $this->mailObj->From = $FROM['email'];
                $this->mailObj->FromName = $FROM['name'];
            }
