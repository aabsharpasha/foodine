...
class Cron extends CI_Controller {
        ...
        foreach ($records as $record) {
            ...
            if((strtoupper($record['eCriteria'])=="ALL") ||(strtoupper($record['eCriteria'])=="AGE" && !empty($record['iMinAge']) && !empty($record['iMaxAge'])) ){
                ...
                foreach($iosDeviceToken as $iosDeviceToken) {
                    ...
                    $this->pushnotification->sendIt(2, $iosDeviceToken['deviceToken'], $message, $pushData);
                    //$this->pushnotify->sendIt(2, $iosDeviceToken['deviceToken'], $message, 2, $pushData);
                }
    ...
    function __sendEmail($to, $subject, $content ){
        ...
        foreach ($records as $record) {
            ...
            foreach($users AS $user){
                if(!empty($user['vEmail'])){
                    $this->__sendEmail($user['vEmail'], $record["vSubject"], $record["tContent"] );
                }
            }
            if(!empty($record["tOtherRecipients"])){
                $emails = explode(",", $record["tOtherRecipients"]);
                foreach($emails AS $email){
                    if(!empty($email)){
                        $this->__sendEmail($email, $record["vSubject"], $record["tContent"] );
                    }
                }
            }
            
            $this->db->query("UPDATE tbl_promotional_emails SET eSent='Yes' WHERE iPromotionalEmailId='".$record['iPromotionalEmailId']."'");
