                            ...
                            <div id="divtoappend" class="row">
                                <div class="col-sm-12">
                                    <div class="page-header">
                                        ...
                                        <div class="description">Team Listing</div>
                                    </div>
                                </div>
                            </div>
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                var target = [
                    {
                        ...
                        "mRender": function (data, type, full) {
                            ...
                            if (permission.indexOf('2') >= 0) {
                                buttons += ' <a title="Edit" href="<?= BASEURL ?>' + controller + '/addTeam/' + full['iMemberID'] + '/y"  class="btn btn-sm btn-primary marginright10 margintop10 "><i class="fa fa-pencil-square-o"></i> Edit </a> ';
                            }
                            if (permission.indexOf('3') >= 0) {
                                buttons += '\n <button title="Delete" class="btn btn-sm btn-danger marginright10 margintop10"  onclick="return validateRemove(' + full['iMemberID'] + ',' + "'" + controller + "/deleteAll'" + ');"><i class="fa fa-times"></i> Delete</button>';
                            }
                            if (permission.indexOf('4') >= 0) {
                                if (full['eStatus'] == "Active") {
                                    buttons += '<a title="Click here to inactive" id="atag' + full['iMemberID'] + '" onclick="return changeStatus(' + full['iMemberID'] + ',' + "'" + controller + "/status/" + full['iMemberID'] + "/y'" + ')"  class="btn btn-sm btn-success  marginright10 margintop10"><i class="fa fa-check-circle-o"></i> Active </a>'
                                } else {
                                    buttons += '<a title="Click here to Active" id="atag' + full['iMemberID'] + '" onclick="return changeStatus(' + full['iMemberID'] + ',' + "'" + controller + "/status/" + full['iMemberID'] + "/y'" + ')"  class="btn btn-sm btn-inverse  marginright10 margintop10"><i class="fa fa-times-circle-o "></i> Inactive </a>'
                                }
                            }
                            return buttons;
                        }
                    },
                    {
                        ...
                        "mRender": function (data, type, full) {
                            ...
                            return full['vMemberTagline'];
                        }
                    },
                    {
                        "aTargets": [0], // Column to target
                        "mRender": function (data, type, full) {
                            return full['vMemberName'];
                        }
                    },
                    {
                    ...
                    },
                    {
                        ...
                        "mRender": function (data, type, full) {
                            if (full['vMemberImage'] != '') {
                                return '<a class="fancybox" rel="group" href="' + imagepath + full['iMemberID'] + '/' + full['vMemberImage'] + '"><img class="thumbnail img-responsive" src="' + imagepath + full['iMemberID'] + '/thumb/' + full['vMemberImage'] + '"  height="70" width="90" /></a>';
                            } else {
                ...
                var aoculumn = [
                    ...
                    /*3*/ {"mData": "iMemberID", bSortable: false, bSearchable: false, "sWidth": "25%"}
                ];
