...
<html lang="en-us">
    ...
    <body>
                            ...
                            <div id="divtoappend" class="row">
                                <div class="col-sm-12">
                                    <div class="page-header">
                                        ...
                                        <div class="description">Events Listing</div>
                                    </div>
                                </div>
                            </div>
        ...
        <script>
            var oTable, controller = '<?php echo $this->controller; ?>', imagepath = '<?php echo EVENT_IMAGE_PATH ?>', no_img_url = '<?php echo IMGURL; ?>/admin/img/no-image.png';
            var url;
                ...
                var target = [
                    {
                        ...
                        "mRender": function (data, type, full) {
                            ...
                            var buttons = ' <a title="Edit" href="<?= BASEURL; ?>' + controller + '/add/' + full['iEventID'] + '/y"  class="btn btn-sm btn-primary marginright10 margintop10 "><i class="fa fa-pencil-square-o"></i> Edit </a> \n <button title="Delete" class="btn btn-sm btn-danger  marginright10 margintop10"  onclick="return validateRemove(' + data + ',' + "'" + controller + "/deleteAll'" + ');"><i class="fa fa-times"></i> Delete</button>';
                            if (full['eStatus'] == "Active") {
                                buttons += '<a title="Click here to inactive" id="atag' + full['iEventID'] + '" onclick="return changeStatus(' + data + ',' + "'" + controller + "/status/" + full['iEventID'] + "/y'" + ')"  class="btn btn-sm btn-success marginright10 margintop10"><i class="fa fa-check-circle-o"></i> Active </a>'
                            } else {
                                buttons += '<a title="Click here to Active" id="atag' + full['iEventID'] + '" onclick="return changeStatus(' + data + ',' + "'" + controller + "/status/" + full['iEventID'] + "/y'" + ')"  class="btn btn-sm btn-inverse marginright10 margintop10"><i class="fa fa-times-circle-o "></i> Inactive </a>'
                            }
                            return buttons;
                        }
                    },
...
//                            } else {
//                                var buttons = ' <a title="View Restaurants" href="<?= BASEURL ?>restaurants/index/' + full['iEventID'] + '" class="btn btn-sm btn-primary marginright10 margintop10 " ><i class="fa fa-pencil-square-o"></i> View Restaurants ( ' + full['total_restaurants'] + ' ) </a>';
//                            }
//                            return buttons;
//                        }
//                    },
                    ...
                    },
                    {
                        ...
                        {
                            if (full['iEventImage'] != '') {
                                return '<a class="fancybox" rel="group" href="' + imagepath + full['iRestaurantId'] + '/' + full['iEventImage'] + '"><img class="thumbnail img-responsive" src="' + imagepath + full['iRestaurantId'] + '/thumb/' + full['iEventImage'] + '"  height="70" width="90" /></a>';
                            } else {
                ...
                var aoculumn = [
                {"mData": "iEventTitle", "sWidth": "10%"},
                {"mData": "iEventImage", "sWidth": "10%"},
                {"mData": "vRestaurantName", "sWidth": "10%"},
                {"mData": "iEventDescription", "sWidth": "10%"},
                {"mData": "dEventStartDate", "sWidth": "10%"},
                {"mData": "dEventEndDate", "sWidth": "10%"},
                {"mData": "vEventStartTime", "sWidth": "10%"},
                {"mData": "iDayofEvent", "sWidth": "10%"},
                {"mData": "iVenueofEvent", "sWidth": "10%"},
                {"mData": "URL", "sWidth": "10%"},
                    /*2*/ {"mData": "tCreatedAt", "sWidth": "10%"},
                    /*3*/ {"mData": "iEventID", bSortable: false, bSearchable: false, "sWidth": "15%"}
                ];
