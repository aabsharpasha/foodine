...
$permission = get_page_permission($ADMINTYPE, 43);
?>
<!doctype html>
<html lang="en-us">
    <head>
    ...
    <body>
        ...
        <script>
            var oTable, controller = '<?php echo $this->controller; ?>', imagepath = '<?php echo REWARD_IMAGE_PATH ?>',
            ...
            $(document).ready(function () {
                ...
                var target = [{
                        ...
                        "mRender": function (data, type, full) {
                            ...
                            if (permission.indexOf('2') >= 0) {
                                buttons += ' <a title="Edit" href="<?= BASEURL ?>' + controller + '/add/' + full['iRewardID'] + '/y"  class="btn btn-primary marginright10 margintop10 "><i class="fa fa-pencil-square-o"></i> Edit </a>';
                            }
                            ...
                            if (permission.indexOf('4') >= 0) {
                                if (full['eStatus'] === "Active") {
                                    buttons += '<a title="Click here to inactive" id="atag' + full['iRewardID'] + '" onclick="return changeStatus(' + data + ',' + "'" + controller + "/status/" + full['iRewardID'] + "/y'" + ')"  class="btn btn-success  btn-sm marginright10 margintop10"><i class="fa fa-check-circle-o"></i> Active </a>'
                                } else {
                                    buttons += '<a title="Click here to Active" id="atag' + full['iRewardID'] + '" onclick="return changeStatus(' + data + ',' + "'" + controller + "/status/" + full['iRewardID'] + "/y'" + ')"  class="btn btn-inverse btn-sm marginright10 margintop10"><i class="fa fa-times-circle-o "></i> Inactive </a>'
                                }
                            }
                        ...
                        "mRender": function (data, type, full)
                        {
                            if (full['vRewardImage'] !== '') {
                                return '<a class="fancybox" rel="group" href="' + imagepath + full['iRewardID'] + '/' + full['vRewardImage'] + '"><img class="thumbnail img-responsive" src="' + imagepath + full['iRewardID'] + '/thumb/' + full['vRewardImage'] + '"  height="70" width="90" /></a>';
                            } else {
                ...
                var aoculumn = [
                    /*0*/ {"mData": "vRewardTitle", "sWidth": "20%"},
                    /*1*/ {"mData": "iRewardPoint", "sWidth": "15%"},
                    /*2*/ {"mData": "iRewardVoucher", "sWidth": "20%"},
                    /*3*/ {"mData": "vRewardImage", "sWidth": "20%"},
                    /*4*/ {"mData": "iRewardID", bSortable: false, bSearchable: false, "sWidth": "20%"}
                ];
