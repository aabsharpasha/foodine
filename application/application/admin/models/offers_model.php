...
class Offers_Model extends CI_Model {
    ...
    function getRestarantId($iComboOffersID){
        if ($iComboOffersID != '') {
            $result = $this->db->query('select iRestaurantID from tbl_combo_offers where iComboOffersID= '.$iComboOffersID);
    ...
    function getComboPurchaseRecords($iComboOffersID) {
        $fields[] = 'tr.vRestaurantName';
        $fields[] = 'tu.vFirstName';
        $fields[] = 'tu.vLastName';
        $fields[] = 'tu.vFullName';
        $fields[] = 'tu.vEmail';
        $fields[] = 'tu.vMobileNo';
        $fields[] = 'tuc.qty';
        $fields[] = 'tuc.tDiscountedPrice';
        $fields[] = 'tuc.iTotal';
        $fields[] = 'tco.vDaysAllow';
        $fields[] = 'tuc.eAvailedStatus';
        $fields[] = 'tuc.iUserComboID as recordId';
        $fields[] = 'tco.iRestaurantID';
        $fields[] = 'tr.iRestaurantID as id';
        $fields[] = 'tco.iComboOffersID';

        $qry = 'SELECT ' . implode(',', $fields) . ' FROM tbl_user_combo as tuc, tbl_user as tu, tbl_combo_offers tco, tbl_restaurant tr WHERE  tuc.iComboOffersID = ' . $iComboOffersID . ' AND tuc.eBookingStatus="Paid" AND tuc.iUserID = tu.iUserID AND tuc.iComboOffersID =tco.iComboOffersID AND tr.iRestaurantID = tco.iRestaurantID';
        $result = $this->db->query($qry);
    ...
    function getComboRedeemRecords($iComboOffersID) {
        ...
        $qry = 'SELECT ' . implode(',', $fields) . ' FROM tbl_user_combo as tuc, tbl_user as tu, tbl_combo_offers tco, tbl_restaurant tr WHERE  tuc.iComboOffersID = ' . $iComboOffersID . ' AND tuc.eBookingStatus="Paid" AND tuc.eAvailedStatus="Availed" AND tuc.iUserID = tu.iUserID AND tuc.iComboOffersID =tco.iComboOffersID AND tr.iRestaurantID = tco.iRestaurantID';
        $result = $this->db->query($qry);
    ...
    function get_paginationresult() {
        ...
        if ($iRestaurantID != 0) {
            $where_condition .= ' AND tco.iRestaurantID = "' . $iRestaurantID . '"';
        }
        $field = "(SELECT COUNT(*) FROM tbl_user_combo AS tuc WHERE tuc.iComboOffersID IN(tco.iComboOffersID) and tuc.eBookingStatus='Paid') AS total_purchase";
        $field .= ", (SELECT COUNT(*) FROM tbl_user_combo AS tuc WHERE tuc.iComboOffersID IN(tco.iComboOffersID) and tuc.eBookingStatus='Paid' and tuc.eAvailedStatus='Availed') AS total_redeem";
        $field .= ", (SELECT COUNT(*) FROM tbl_offer_menu_image AS tomi WHERE tomi.iComboOffersID IN(tco.iComboOffersID) and tomi.eStatus='Active') AS total_menu_images";
        $data = $this->datatableshelper->query("SELECT tco.vOfferText as vOfferText, vOfferImage,  vRestaurantName, vDaysAllow,tr.iRestaurantID as iRestaurantID, tco.dtStartDate as dtStartDate1  , tco.dtExpiryDate as dtExpiryDate1 ,  DATE_FORMAT(tco.dtStartDate , '%d %b %Y') as dtStartDate, DATE_FORMAT(tco.dtExpiryDate , '%d %b %Y') as dtExpiryDate, tco.eStatus  as eStatus , tco.iComboOffersID as iComboOffersID ,".$field.", iComboOffersID AS DT_RowId FROM tbl_combo_offers AS tco, tbl_restaurant AS tr WHERE tco.iRestaurantID = tr.iRestaurantID " . $where_condition);
        return $data;
    }
    ...
    function get_paginationresultsuboffers($iComboOffersID) {
        ...
        $data = $this->datatableshelper->query("SELECT tcso.vOfferTitle as vOfferTitle, tOfferDetail,  tActualPrice, tDiscountedPrice,tco.iComboOffersID as iComboOffersID,iComboSubOffersID, iComboSubOffersID AS DT_RowId,tco.vOfferText as vOfferText, tcso.dCommission AS dCommission FROM tbl_combo_sub_offers AS tcso, tbl_combo_offers AS tco WHERE tcso.iComboOffersID = tco.iComboOffersID " . $where_condition);
        return $data;
    }

    function get_paginationresulthistory(){
        $where_condition = '';
        $iRestaurantID = (int) $this->session->userdata('iRestaurantID');
        if ($iRestaurantID != 0) {
            $where_condition .= ' AND tr.iRestaurantID = "' . $iRestaurantID . '"';
        }
        $fields     = array();
        $fields[]   = 'tr.vRestaurantName AS vRestaurantName';
        $fields[]   = "CONCAT(tu.vFirstName,' ',tu.vLastName) AS userName";
        $fields[]   = 'tu.vEmail AS vEmail';
        $fields[]   = 'tu.vMobileNo AS vMobileNo';
        $fields[]   = 'tuc.qty AS qty';
        $fields[]   = 'tuc.tDiscountedPrice AS tDiscountedPrice';
        $fields[]   = 'tuc.iTotal AS iTotal';
        $fields[]   = 'tuc.eAvailedStatus AS eAvailedStatus';
        $fields[]   = 'tuc.iComboSubOffersID AS iComboSubOffersID';
        $fields[]   = 'tuc.iRestaurantID AS iRestaurantID';
        $fields[]   = 'tuc.iUserComboID AS iUserComboID';
        $fields[]   = "CONCAT(tcso.vOfferTitle,': <BR> ',tcso.tOfferDetail) AS offerText";
        $fieldsStr  = implode(", ", $fields);
        
        $query      = 'SELECT '.$fieldsStr.' FROM tbl_user_combo as tuc, tbl_user as tu, tbl_combo_sub_offers tcso, tbl_restaurant tr WHERE  tuc.eBookingStatus="Paid" AND tuc.iUserID = tu.iUserID '.$where_condition.' AND tuc.iComboSubOffersID =tcso.iComboSubOffersID AND tuc.iRestaurantID = tr.iRestaurantID';
    ...
    function removeDeals($iDealID) {
        ...
        $query1 = $this->db->query("DELETE from tbl_combo_sub_offers WHERE iComboOffersID IN ($adid) ");
        if ($this->db->affected_rows() > 0) {
            ...
            $ids = $iDealID;
            if (!empty($ids)) {
                $basePath = DOC_ROOT . '/images/combo/';
    ...
    function removeSubDeals($iDealID) {
        $adid = implode(',', $iDealID);

        $query = $this->db->query("DELETE from tbl_combo_sub_offers WHERE iComboSubOffersID IN ($adid) ");
        if ($this->db->affected_rows() > 0) {
            return $query;
        } else {
            return '';
...
//        }
    ...
    function changeUserComboStatus($iUserComboID) {
        $query = $this->db->query("UPDATE tbl_user_combo SET eAvailedStatus = IF (eAvailedStatus = 'Availed', 'Not Availed','Availed') WHERE iUserComboID = '" . $iUserComboID . "'");
        if ($this->db->affected_rows() > 0)
            return $query;
        else
            return '';
    ...
    function offerMenuById($iPictureID){
        if ($iPictureID !== '') {
            $res = $this->db->get_where('tbl_offer_menu_image', array('iMenuPictureID' => $iPictureID));
    ...
    function add_offer_menu($postValues = array()) {

        if (!empty($postValues)) {

            $combo_id   = $postValues['iComboOffersID'];
            $menu_type  = $postValues['menu_type'];
            $images     = $postValues['images'];

            for ($i = 0; $i < count($images); $i++) {
                $insArry = array(
                    'iComboOffersID'=> $combo_id,
                    'eMenuType'     => $menu_type,
                ...
                $this->db->insert('tbl_offer_menu_image', $insArry);
            }
        } else {
            return '';
        }
    ...
    function edit_offer_menu($postValues = array()) {
        if (!empty($postValues)) {
            ...
            for ($i = 0; $i < count($images); $i++) {
                ...
                $this->db->update('tbl_offer_menu_image', $updtArry, array('iMenuPictureID' => $mpic_id));
            }
        } else {
            return '';
        }
    ...
    function offer_menu_paginate($iComboOffersID) {
        $qry = "SELECT vPictureName, eMenuType, DATE_FORMAT(tCreatedAt , '%d %b %Y %h:%i %p') as tCreatedAt, eStatus, iMenuPictureID, iComboOffersID , iMenuPictureID AS DT_RowId FROM tbl_offer_menu_image as r where iComboOffersID = $iComboOffersID";
        $data = $this->datatableshelper->query($qry);
        return $data;
    }
    
    function changeImageStatusOfferMenu($iPictureID) {
        $query = $this->db->query("UPDATE tbl_offer_menu_image SET eStatus = IF (eStatus = 'Active', 'Inactive','Active') WHERE iMenuPictureID = $iPictureID");
        if ($this->db->affected_rows() > 0)
            return $query;
        else
            return '';
    ...
    function removeOfferMenu($rows) {
        $adid = implode(',', $rows);
        $res_data = array();
        $result = $this->db->query("SELECT iComboOffersID , iMenuPictureID , vPictureName FROM tbl_offer_menu_image WHERE iMenuPictureID IN (" . $adid . ")");
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            foreach ($row as $value) {
                ...
                $baseDIR = DOC_ROOT . "/images/comboOfferMenu/" . $value['iComboOffersID'] . "/";
                $this->unlinkimage($baseDIR . $value['vPictureName']);
                $this->unlinkimage($baseDIR . "thumb/" . $value['vPictureName']);
            }
            $this->db->query("DELETE FROM tbl_offer_menu_image WHERE iMenuPictureID IN (" . $adid . ")");
            return $row;
        } else
            return '';
    }
