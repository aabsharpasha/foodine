...
class Image_model extends CI_Model {
    ...
    function restaurant_photo_paginate($iRestaurantID) {
        $qry = "SELECT vPictureName , DATE_FORMAT(tCreatedAt , '%d %b %Y %h:%i %p') as tCreatedAt, eStatus, iPictureID, iRestaurantID , iPictureID AS DT_RowId FROM tbl_restaurant_image as r where iRestaurantID = $iRestaurantID";
        $data = $this->datatableshelper->query($qry);
        return $data;
    }

    function restaurant_menu_paginate($iRestaurantID) {
        $qry = "SELECT vPictureName, eMenuType, DATE_FORMAT(tCreatedAt , '%d %b %Y %h:%i %p') as tCreatedAt, eStatus, iMenuPictureID, iRestaurantID , iMenuPictureID AS DT_RowId FROM tbl_restaurant_menu_image as r where iRestaurantID = $iRestaurantID";
        $data = $this->datatableshelper->query($qry);
        return $data;
    }
