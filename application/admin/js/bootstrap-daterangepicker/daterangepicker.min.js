!function(e) {
    var t = function(t, n, r) {
        ...
        this.parentEl = i && n.parentEl && e(n.parentEl) || e(this.parentEl);
        this.container = e(o).appendTo(this.parentEl);
        if (i) {
            ...
            if (typeof n.startDate == "string")
                this.startDate = moment(n.startDate, this.format);
            if (typeof n.endDate == "string")
                this.endDate = moment(n.endDate, this.format);
            if (typeof n.minDate == "string")
                this.minDate = moment(n.minDate, this.format);
            if (typeof n.maxDate == "string")
                this.maxDate = moment(n.maxDate, this.format);
            if (typeof n.startDate == "object")
                this.startDate = moment(n.startDate);
            if (typeof n.endDate == "object")
                this.endDate = moment(n.endDate);
            if (typeof n.minDate == "object")
                this.minDate = moment(n.minDate);
            if (typeof n.maxDate == "object")
                this.maxDate = moment(n.maxDate);
            if (typeof n.ranges == "object") {
                for (var u in n.ranges) {
                    var a = moment(n.ranges[u][0]);
                    var f = moment(n.ranges[u][1]);
                    if (this.minDate && a.isBefore(this.minDate))
                        a = moment(this.minDate);
                    if (this.maxDate && f.isAfter(this.maxDate))
    ...
    t.prototype = {constructor: t, mousedown: function(e) {
            ...
            var t = moment(e[0], this.format);
            var n = moment(e[1], this.format);
            if (t == null || n == null)
                return;
            if (n.isBefore(t))
                return;
            ...
            var t = {top: this.parentEl.offset().top - (this.parentEl.is("body") ? 0 : this.parentEl.scrollTop()), left: this.parentEl.offset().left - (this.parentEl.is("body") ? 0 : this.parentEl.scrollLeft())};
            if (this.opens == "left") {
                this.container.css({top: this.element.offset().top + this.element.outerHeight() - t.top, right: e(window).width() - this.element.offset().left - this.element.outerWidth() - t.left, left: "auto"});
                if (this.container.offset().left < 0) {
                    this.container.css({right: "auto", left: 9})
                }
            } else {
                this.container.css({top: this.element.offset().top + this.element.outerHeight() - t.top, left: this.element.offset().left - t.left, right: "auto"});
                if (this.container.offset().left + this.container.outerWidth() > e(window).width()) {
            ...
            if (s.hasClass("left")) {
                this.container.find("input[name=daterangepicker_start]").val(this.leftCalendar.calendar[r][i].format(this.format))
            } else {
                this.container.find("input[name=daterangepicker_end]").val(this.rightCalendar.calendar[r][i].format(this.format))
            }
            ...
            if (s.hasClass("left")) {
                ...
                var u = this.rightCalendar.calendar[r][i];
                if (typeof this.dateLimit == "object") {
            ...
            var r = this.container.find(".calendar.left");
            if (!n)
                r = this.container.find(".calendar.right");
            var i = parseInt(r.find(".monthselect").val());
            var s = r.find(".yearselect").val();
            if (n) {
                this.leftCalendar.month.month(i).year(s)
            } else {
                this.rightCalendar.month.month(i).year(s)
            }
            this.updateCalendars()
            ...
            var s = parseInt(r.find(".minuteselect").val());
            ...
            for (var n in this.ranges) {
                if (this.timePicker) {
                    ...
                    if (this.startDate.format("YYYY-MM-DD") == this.ranges[n][0].format("YYYY-MM-DD") && this.endDate.format("YYYY-MM-DD") == this.ranges[n][1].format("YYYY-MM-DD")) {
                        e = false;
                        this.container.find(".ranges li:eq(" + t + ")").addClass("active")
                    }
                }
            ...
            var s = moment([t, e, 1]);
            var o = moment(s).subtract("month", 1).month();
            var u = moment(s).subtract("month", 1).year();
            var a = moment([u, o]).daysInMonth();
            var f = s.day();
            var l = [];
            for (var c = 0; c < 6; c++) {
                l[c] = []
            ...
            if (f == this.locale.firstDay)
                h = a - 6;
            var p = moment([u, o, h, 12, r]);
            for (var c = 0, d = 0, v = 0; c < 42; c++, d++, p = moment(p).add("hour", 24)) {
                if (c > 0 && d % 7 == 0) {
                    d = 0;
                    v++
                }
                l[v][d] = p.clone().hour(n);
                p.hour(12)
            }
            return l
            ...
            if (this.showWeekNumbers)
                s += "<th></th>";
            if (!r || r.isBefore(t[1][1])) {
                s += '<th class="prev available"><i class="icon-arrow-left glyphicon glyphicon-arrow-left"></i></th>'
            } else {
                s += "<th></th>"
            }
            ...
            if (!i || i.isAfter(t[1][1])) {
                s += '<th class="next available"><i class="icon-arrow-right glyphicon glyphicon-arrow-right"></i></th>'
            } else {
                s += "<th></th>"
            }
            s += "</tr>";
            ...
            for (var u = 0; u < 6; u++) {
                ...
                for (var a = 0; a < 7; a++) {
                    ...
                    if (r && t[u][a].isBefore(r) || i && t[u][a].isAfter(i)) {
                        ...
                        f += " active ";
                        if (t[u][a].format("YYYY-MM-DD") == this.startDate.format("YYYY-MM-DD")) {
                            f += " start-date "
                        }
                        if (t[u][a].format("YYYY-MM-DD") == this.endDate.format("YYYY-MM-DD")) {
                            f += " end-date "
                        }
                    } else if (t[u][a] >= this.startDate && t[u][a] <= this.endDate) {
                        f += " in-range ";
                        if (t[u][a].isSame(this.startDate)) {
                            f += " start-date "
                        }
                        if (t[u][a].isSame(this.endDate)) {
                            f += " end-date "
                        }
                    }
                    var l = "r" + u + "c" + a;
            ...
            if (this.timePicker) {
                ...
                for (var d = c; d <= h; d++) {
                    if (d == p) {
                        s += '<option value="' + d + '" selected="selected">' + d + "</option>"
                    } else {
                ...
                for (var d = 0; d < 60; d += this.timePickerIncrement) {
                    ...
                    if (v < 10)
                        v = "0" + v;
                    if (d == n.minute()) {
                        s += '<option value="' + d + '" selected="selected">' + v + "</option>"
                    } else {
                        s += '<option value="' + d + '">' + v + "</option>"
                    }
                }
                s += "</select> ";
                if (this.timePicker12Hour) {
                    ...
                    if (n.hour() >= 12) {
                        s += '<option value="AM">AM</option><option value="PM" selected="selected">PM</option>'
                    } else {
                        s += '<option value="AM" selected="selected">AM</option><option value="PM">PM</option>'
                    }
                    s += "</select>"
                }
                s += "</div>"
